...
if (!class_exists("Element_Pack_Base")) {
    class Element_Pack_Base
    {
        ...
        private function _request($relative_url, $data, &$error = '')
        {
            ...
            // Ensure WordPress HTTP API is loaded
            if (!function_exists('wp_remote_post')) {
                // Try to load WordPress HTTP API
            ...
            if (function_exists('wp_remote_post')) {
                ...
                // Helper function to check for IP blocking
                $check_ip_blocking = function($server_response) use ($response) {
                    $response_code = wp_remote_retrieve_response_code($server_response);
                    if ($response_code === 403 && !empty($server_response['body'])) {
                        $body = $server_response['body'];
                        // Extract IP address from the response
                        $ip_pattern = '/banned your IP address \((\d+\.\d+\.\d+\.\d+)\)/';
                        $error_code_pattern = '/errorCode:\s*(\d+)/';
                        
                        $blocked_ip = '';
                        $error_code = '';
                        
                        if (preg_match($ip_pattern, $body, $matches)) {
                            $blocked_ip = $matches[1];
                        }
                        
                        if (preg_match($error_code_pattern, $body, $matches)) {
                            $error_code = $matches[1];
                        }
                        
                        if ($blocked_ip) {
                            $response->msg = "Your IP address {$blocked_ip} has been blocked by Cloudflare (Error Code: {$error_code}). Please contact support";
                        } else {
                            $response->msg = "Your access to license server was blocked by Wordfence. Please contact support.";
                        }
                        
                        $response->status = false;
                        $response->data = NULL;
                        $response->is_request_error = true;
                        $response->is_ip_blocked = true;
                        $response->cloudflare_error_code = $error_code;
                        $response->blocked_ip = $blocked_ip;
                        return true; // IP blocking detected
                    }
                    return false; // No IP blocking
                };
                ...
                if (is_wp_error($server_response)) {
                    ...
                    if (is_wp_error($server_response)) {
                        ...
                        // Check for IP blocking after retry
                        if ($check_ip_blocking($server_response)) {
                            return $response;
                        }
                    ...
                    // Check for IP blocking on first attempt
                    if ($check_ip_blocking($server_response)) {
                        return $response;
                    }
